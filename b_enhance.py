...
def _02_segment(fingerprint, block_size, std_threshold, view=False):
    ...
    # Create a mask, starts with all zeroes
    ...
    for row in range(h):
        for col in range(w):
            ...
            # This means you want to keep the pixels in that block. 255 = the color white
            # Check std. Dev. for each block and if it's higher than .25, keep it.
            if numpy.std(block) > std_threshold:
                mask[row, col] = 255

    # Remove too low / too high pixel values
...
def _03_compute_orientations(fingerprint, mask, block_size, view=False):
    h, w = fingerprint.shape
    # Calculate gradient
    # function auto calculates x and y gradients
    y_gradient, x_gradient = numpy.gradient(fingerprint)

    # Orientation and magnitude calculated below
